...
class Flow_Control():
    ...
    def all_calls(self):
        ...
        if self.state_dict['colocalization'] == True:
            ...
            # st.write(spearman_stats)
            manders_coeff = coloc.manders()
...
class Setup():
    ...
    def create_table(self, df, title):
        ...
        fig.update_layout(title = title, title_font = dict(size = 18, color = 'green',
                family = 'Arial'), title_x = 0, title_y = 0.95, height = 185,
...
class Prep_Image():        
    ...
    def cropping(self):
        ...
        st.markdown('<p class = "purple-font-venti">Use the cropper box to select an area to process.</p>', unsafe_allow_html = True) 
        self.cropped_img = st_cropper(self.state_dict['original image'], realtime_update = True, box_color = '#0000FF',
                                    aspect_ratio = (1,1))
    ...
    def denoise(self):
        ...
        # Denoise the image
        self.denoised_img_array = cv.fastNlMeansDenoising(self.state_dict['cropped image array'], None, 15, 7, 21)  
   ...
   # Applies an image enhancer
    def enhance_details(self, img_array):
        ...
        self.enhanced_img_array = cv.detailEnhance(img_array, sigma_s = self.state_dict["sigma_s"], sigma_r = self.state_dict["sigma_r"])
    ...
    def brighten(self, img_array):
        ...
        self.brightened_image_array = cv.convertScaleAbs(img_array, beta = self.state_dict['brightness']) 
    ...
    def threshold_image(self, img_array):
        ...
        try:
            ...
            ret, self.thresh = cv.threshold(self.gray_img, self.state_dict['threshold'], 255, cv.THRESH_BINARY)
    ...
    def download_image(self, image, btn_text):
        """Wanted to put this in the Setup class but it created a lot of padding above the button for some reason."""
        pilImage = Image.fromarray(image)
        buffer = io.BytesIO()
        pilImage.save(buffer, format = "PNG")
        btn = st.download_button(
...
class Gauss_Test():
    ...
    def gaussian_test(self):
        ...
        stats.probplot(np.array(self.state_dict['flat red']), dist = 'norm', plot = axs[0])
        axs[0].set_title('Red Channel')
        stats.probplot(np.array(self.state_dict['flat green']), dist = 'norm', plot = axs[1])
        axs[1].set_title('Green Channel')
        stats.probplot(np.array(self.state_dict['flat blue']), dist = 'norm', plot = axs[2])
...
class Colocalization():        
    ...
    def get_index_vals(self, box_height, color1, color2):
        ...
        # key_name = color1 + ' - ' + color2 + '_min_index'
    ...
    def pearson_plot_corrected_data(self): 
        ...
        if {'total red - red from green_subtraction','total green - green from red_subtraction'} <= self.bleed_subtraction_dict.keys():
        ...
        if {'total blue - blue from green_subtraction','total green - green from blue'} <= self.bleed_subtraction_dict.keys():
        ...
        if {'total blue - blue from red_subtraction','total red - red from blue_subtraction'} <= self.bleed_subtraction_dict.keys():
    ...
    def manders(self):
        ...
        rgb_df = pd.DataFrame({'Red':self.state_dict['flat red'], 'Green':self.state_dict['flat green'], 
                               'Blue':self.state_dict['flat red']})
    ...
    def manders_table(self, manders):
        ...
        fig.update_layout(title = 'Manders Co-occurance Results', title_font = dict(size = 18, color = 'green',
                          family = 'Arial'), title_x = 0.0, title_y = 0.95, height = 175,
...
class Cell_Counter():
    ...
    def contour(self):
        ...
        self.contours, self.hierarchy = cv.findContours(self.state_dict['threshold image'], cv.RETR_LIST, cv.CHAIN_APPROX_NONE)
    ...
    def cell_stats(self):
        ...
        for contour in self.state_dict['contours']:
            ...
            if cv.contourArea(contour) > self.state_dict['contour_area']:
                ...
                cv.ellipse(self.state_dict['cropped image array'], ellipse, (255,255,255), 2, cv.LINE_AA)
                
                # Draw the contour that follows the cell perfectly
                cv.drawContours(self.state_dict['cropped image array'], contour, -1, (0, 255, 255), 2)
                ...
                cx = int(M['m10']/M['m00'])
                cy = int(M['m01']/M['m00'])
                cv.putText(self.state_dict['cropped image array'], text = str(cell_number), org=(cx,cy),
                        fontFace = cv.FONT_HERSHEY_SIMPLEX, fontScale = 2, color = (255,255,255),
                        thickness = 2, lineType=cv.LINE_AA)
        
        # Print the real image with contours and labels
        st.image(self.state_dict['cropped image array'], use_column_width = True, clamp = True)
        
        # Download the image with the contours marked by calling download_image method in Prep_Image class
        Prep_Image(state_dict).download_image(self.state_dict['cropped image array'], "Download Cell Contours")
        ...
        fig = plt.figure(figsize = (7, 2))
        red_hist = plt.hist(red_list, color = 'red', bins = 50, label = 'Red Channel')
        green_hist = plt.hist(green_list, color = 'green', bins = 50, label = 'Green Channel')
        ...
        st.write('')
        st.write('')
        live_dead_threshold = st.slider('Select an intensity threshold for Live-Dead Analysis', 75, 200, 100, 5)
        ...
        fig.update_layout(title = "Cell Analysis", title_font = dict(size = 18, color = 'green',
                family = 'Arial'), title_x = 0, title_y = 0.95, height = 300, width = 1000,
                          margin = dict(l = 0, r = 0, b = 0, t = 35))
        st.write('')
        st.write('')
        st.write(fig)      
        ...
        fig.update_layout(title = "Cell Totals", title_font = dict(size = 18, color = 'green',
                family = 'Arial'), title_x = 0, title_y = 0.95, height = 100, width = 1000,
